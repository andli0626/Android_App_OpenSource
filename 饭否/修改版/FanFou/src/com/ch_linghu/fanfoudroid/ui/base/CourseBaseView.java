 ...
 */
public abstract class CourseBaseView extends ListBaseView {
	static final String TAG = "TwitterCursorBaseActivity";

	// Views.
	protected PullToRefreshListView mListView;// 列表
	...
	protected ProgressBar loadMoreGIFProgressBar;
	protected TextView loadMoreBtnTop;
	protected ProgressBar loadMoreGIFProgressBarTop;
	...
	private ITaskListener mRetrieveTaskListener = new AbstractTaskAdapter() {
		...
		public void onPostExecute(GenericTask task, TaskResult result) {
			// 刷新按钮停止旋转
			loadMoreGIFProgressBar.setVisibility(View.GONE);
	...
	// private static final long FOLLOWERS_REFRESH_THRESHOLD = 12 * 60 * 60 *
	// 1000;
	...
	protected void setupState() {
		...
		mListView = (PullToRefreshListView) findViewById(R.id.tweet_list);
	...
	protected void setupListHeader(boolean addFooter) {
		...
		loadMoreGIFProgressBar = (ProgressBar) findViewById(R.id.rectangleProgressBar);
		loadMoreBtnTop = (TextView) findViewById(R.id.ask_for_more_header);
		loadMoreGIFProgressBarTop = (ProgressBar) findViewById(R.id.rectangleProgressBar_header);
	...
	protected void specialItemClicked(int position) {
		...
		if (position == 0) {// 第一个Item(header)
			loadMoreGIFProgressBarTop.setVisibility(View.VISIBLE);
			doRetrieve();
		} else if (position == mListView.getCount() - 1) {// 最后一个Item(footer)
			loadMoreGIFProgressBar.setVisibility(View.VISIBLE);
			doGetMore();
		}
	}
	...
	protected void onDestroy() {
		...
		// 刷新按钮停止旋转
		if (loadMoreGIFProgressBar != null) {
			loadMoreGIFProgressBar.setVisibility(View.GONE);
		}
	...
	{
		useGestrue = AppParam.mPref.getBoolean(Preferences.USE_GESTRUE, false);
		if (useGestrue) {
	...
	{
		useShake = AppParam.mPref.getBoolean(Preferences.USE_SHAKE, false);
		if (useShake) {
