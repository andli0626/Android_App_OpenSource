...
public class RadioAdp extends BaseAdp<Radio> {

	int mIconSize;

	public RadioAdp(Activity context) {
		super(context);
		mIconSize = (int) context.getResources()
				.getDimension(R.dimen.icon_size);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (row == null) {
			...
			holder.image = (MyImageView) row
					.findViewById(R.id.PurpleImageView);
		...
		if (mList.get(position).getName().length() == 0
				|| mList.get(position).getName().equals("null")) {
		...
		holder.image.setImageUrl(mList.get(position).getImage(), position,
				getListView());
